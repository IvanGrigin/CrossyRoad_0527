...
public class Man {
    ...
    public boolean checkCollisionRoad(Road r) {
        boolean ret = false;
        if (Math.abs(this.y - r.y) < 35) {       //входит на дорогу (опасная полоса)
            for (int i = 0; i < r.cars.size(); i++) {  //"дай мне такую-то машину из писка машин на дороге"
    ...
    public boolean checkCollisionLily(WaterLily b) {  //проверка: стоим на кувшинке или нет?
    ...
    public boolean checkCollisionRiver(River r) {  //входит на реку (опасная полоса)
        boolean ret = false;
    ...
    public boolean checkCollisionBridge(Bridge b) {  //проверка: стоим на мосту или нет?
        if ((this.x + this.w/2 < b.x + b.w) && (this.x + this.w/2 > b.x) && (this.y + 2 < b.y + b.h) && (this.y + 2 > b.y)) {
            return true;
        } else {
            return false;
