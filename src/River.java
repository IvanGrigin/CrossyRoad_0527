...
public class River {
    ...
    public void addBridges(int level){
        int otstup = this.h;
        int t = (int)(Math.random() * 7); //расстояние между мостами (не больше 7)
        bridges.add(new Bridge((t + 1) * otstup, this.y, otstup, level));
        while (true){
            t = (int)(Math.random() * 7);
            if (bridges.get(bridges.size() - 1).x + 1.5 * bridges.get(bridges.size() - 1).w > 600){  //нормально ли навставлялись мосты
                break;
            } else {
