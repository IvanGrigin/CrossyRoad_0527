...
public class WorldPanel extends JPanel {
    ...
    public void CheckKeyEvent(KeyEvent e) {
        if (e.getID() == KeyEvent.KEY_PRESSED) {
            if ((e.getKeyCode() == KeyEvent.VK_ESCAPE) || (e.getKeyCode() == KeyEvent.VK_Q)) {
                inventarPanel.isOn = !inventarPanel.isOn;  //закроем открытую
            } else {
                man.CheckKeyEvent(e);
            }
            if (e.getKeyCode() == KeyEvent.VK_G) {
                isGod = !isGod;  //режим Бога
            }
            if (e.getKeyCode() == KeyEvent.VK_C){
                isContour = !isContour;  //контур
            }
        }
    }
    ...
    public void updateState(long dt) {
        ...
        if (inventarPanel.isOn == false) {
            ...
            man.x = man.x + man.lilySpeed * dt;  //двигается вместе с лилией
            man.x = man.x + 2 * 600;
            man.x = man.x % 600;
        }
    }
