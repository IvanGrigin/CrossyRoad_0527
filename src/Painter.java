...
public class Painter {
    ...
    public void drawMan(Graphics2D g2d, Man man, boolean b){
        int p = Integer.parseInt(man.numberOfImage) - 1;  //номер картинки (выбор персонажа)
        Image test = characters.get(p);
        g2d.drawImage(test, (int) man.x, man.y, man.w, man.h, null);  //отрисовываем картинку
        if (b == true){
    ...
    public void drawRoad(Graphics2D g2d, Road road, boolean b){
        ...
        while (true){
            ...
            k++;
            if (road.x + road.h * k > 800){  //много клеток в дороге нарисовали или мало (проверка)
                break;
            }
        }
